...
class Strcaseto < Formula
  ...
  license "Apache-2.0"
  head "https://github.com/go-curses/coreutils-strcaseto.git", branch: "release-v0.1.0"

  depends_on "go" => :build

  def install
    ENV.deparallelize  # if your formula fails when building in parallel
    system "make", "prefix=#{prefix}", "release", "install", "install-autocomplete", "install-shortlinks"
  end
